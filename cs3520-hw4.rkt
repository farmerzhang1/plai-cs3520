...
(define override-store0 cons)
(define (override-store [new : Storage] [store : Store])
  (type-case (Listof Storage) store
    [empty (list new)]
    [(cons s rest) (if (eq? (cell-location new) (cell-location s))
                       (cons new rest)
                       (cons s (override-store new rest)))]))
...
;; beginE helper function (should be inside interp(((
(define (interp-begin-args [args : (Listof Exp)] [env : Env] [sto : Store]) : Result
  (type-case (Listof Exp) args
    ...
    [empty (error 'interp-begin "shouldn't get here")]))
...
(define (interp [a : Exp] [env : Env] [sto : Store]) : Result
  (type-case Exp a
    ...
    [(plusE l r)
     (with [(v-l sto-l) (interp l env sto)]   ;; v-l 和 sto-l 為 interp l env sto 的結果
